...
const TrackingClient = ({ locale }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("API Response:", foundOrder);
        ...
        <Modal.Body>
          {orderLocation && (
            <div className="order-status-timeline">
              {(() => {
                const finalStatuses = ["delivered", "failed", "returned"];

                if (finalStatuses.includes(orderLocation.status)) {
                  return (
                    <div className="status-step" key={orderLocation.status}>
                      <div
                        className="circle active"
                        style={{
                          backgroundColor:
                            orderLocation.status === "delivered"
                              ? "#28a745"
                              : orderLocation.status === "failed"
                              ? "#dc3545"
                              : "#ffc107",
                        }}
                      >
                      ...
                      <span className="status-label">
                        {statusMap[orderLocation.status]}
                      </span>
                    </div>
                  );
                }
                ...
                return allStatuses.map((status, index, arr) => {
