import ApiClient from "@/Services/APIs";
import AboutClient from "./AboutClient";
import seoConfig from "@/config/seoConfig";
export const dynamic = "force-dynamic";
export const revalidate = 0;
export async function generateMetadata({ params }) {
  const locale = params?.locale || "en";
  const seo = seoConfig.about[locale] || seoConfig.about.en;

  return {
    title: seo.title,
    description: seo.description,
    keywords: seo.keywords,
    canonical: seo.canonical,

    alternates: {
      canonical: seo.canonical,
      languages: {
        en: seoConfig.about.en.url,
        ar: seoConfig.about.ar.url,
        "x-default": seoConfig.about.en.url,
      },
    },

    openGraph: {
      title: seo.openGraph?.title || seo.title,
      description: seo.openGraph?.description || seo.description,
      url: seo.url,
      siteName: seo.openGraph?.siteName || "Cannata",
      locale: seo.openGraph?.locale || (locale === "ar" ? "ar_AE" : "en_US"),
      type: seo.openGraph?.type || "website",
      images: [
        {
          url: seo.openGraph?.image || "/og-images/cannata-shipping-about.jpg",
          width: 1200,
          height: 630,
          alt: seo.title,
        },
      ],
    },

    twitter: {
      card: seo.twitterCard?.card || "summary_large_image",
      title: seo.twitterCard?.title || seo.title,
      description: seo.twitterCard?.description || seo.description,
      images: [seo.twitterCard?.image || "/twitter-images/cannata-about.jpg"],
    },

    other: {
      keywords: seo.keywords,
    },
  };
}

export default async function AboutPage({ params }) {
  const res = await ApiClient.get("about");
  const about = res?.data || [];
  const locale = params.locale;
  const seoData = seoConfig.home[locale] || seoConfig.home.en;

  const jsonLd = {
    ...seoData.schema,
    url: seoData.url,
  };

  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(jsonLd),
        }}
      />
      <AboutClient about={about} locale={locale} />
    </>
  );
}
